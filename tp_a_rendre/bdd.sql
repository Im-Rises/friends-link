...
CREATE TABLE membre (adresse_mail VARCHAR(320) PRIMARY KEY NOT NULL, nom VARCHAR(50) NOT NULL , prenom VARCHAR(50) NOT NULL, date_naissance DATE NOT NULL, mdp TEXT NOT NULL);
...
CREATE TABLE ami(email VARCHAR(320), email_ami VARCHAR(320), amitie_validee BOOLEAN NOT NULL, PRIMARY KEY (email, email_ami), FOREIGN KEY (email) REFERENCES membre(adresse_mail), FOREIGN KEY (email_ami) REFERENCES membre(adresse_mail));
